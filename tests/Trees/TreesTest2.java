...
public class TreesTest2 extends DukeTest {
  ...
  public void start(Stage stage) throws IOException {
    thisFileReader = new ReadCSVFile("Examples/Trees/Test2.csv");
  ...
  public void testFile() throws IOException {
    ...
    int[][] testArray = {{0,0,0},{0,1,0},{0,0,0}};
  ...
  public void testGridModel() throws IOException {
    javafxRun(() -> {
      try {
        test1TreesController.restartSimulationWithNewProp("Trees2.properties");
      } catch (IOException e) {
        e.printStackTrace();
      }
    });
  ...
  public void testGridView() throws IOException {
    ...
    Paint empty = Color.YELLOW;
    Paint tree = Color.GREEN;
    Paint fire = Color.RED;

    GridView thisGridView = test1TreesController.getGridView();

    assertEquals(empty, thisGridView.getRectangle(0,0).getFill());
    assertEquals(empty, thisGridView.getRectangle(0,1).getFill());
    assertEquals(empty, thisGridView.getRectangle(0,2).getFill());
    assertEquals(empty, thisGridView.getRectangle(1,0).getFill());
    assertEquals(tree, thisGridView.getRectangle(1,1).getFill());
    assertEquals(empty, thisGridView.getRectangle(1,2).getFill());
    assertEquals(empty, thisGridView.getRectangle(2,0).getFill());
    assertEquals(empty, thisGridView.getRectangle(2,1).getFill());
    assertEquals(empty, thisGridView.getRectangle(2,2).getFill());
