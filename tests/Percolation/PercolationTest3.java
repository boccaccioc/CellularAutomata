...
public class PercolationTest3 extends DukeTest {
  private  Controller test1Percolution;
  ...
  public void start(Stage stage) throws IOException {
    thisFileReader = new ReadCSVFile("Examples/Percolation/Test3.csv");
    thisGridData = new GridDataArray(thisFileReader.getGrid());
    test1Percolution = new Controller();
    test1Percolution.start(new Stage());
  }
  ...
  public void testFile() throws IOException {
    assertEquals(2,thisFileReader.getGrid()[0][0]);
    int[][] testArray = {{2,1,0},{0,1,0},{0,1,0}};
  ...
  public void testGridData() throws IOException {
    ...
    List<Integer> knownNeighbors = new ArrayList<Integer>(Arrays.asList(2,1,0,0,0,0,1,0));
    ...
    List<Integer> knownNeighbors2 = new ArrayList<Integer>(Arrays.asList(1,0,0,1));
  ...
  public void testGridModel() throws IOException {
    ...
    javafxRun(() -> {
      try {
        test1Percolution.restartSimulationWithNewProp("Percolation3.properties");
      } catch (IOException e) {
        e.printStackTrace();
      }
    });
    GridModel thisGridModel = test1Percolution.getGridOfCells();
    ...
    javafxRun(() -> test1Percolution.step());
    ...
    assertEquals(0, thisGridModel.getCell(1,2).getState());
    assertEquals(0, thisGridModel.getCell(2,0).getState());
    assertEquals(2, thisGridModel.getCell(2,1).getState());
    assertEquals(0, thisGridModel.getCell(2,2).getState());
  }
  ...
  public void testGridView() throws IOException {
    ...
    });    Paint blocked = Color.GREY;
    Paint empty = Color.WHITE;
    Paint full = Color.BLUE;


    GridView thisGridView = test1Percolution.getGridView();

    assertEquals(full, thisGridView.getRectangle(0,0).getFill());
    assertEquals(empty, thisGridView.getRectangle(0,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(0,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(1,0).getFill());
    assertEquals(empty, thisGridView.getRectangle(1,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(1,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,0).getFill());
    assertEquals(empty, thisGridView.getRectangle(2,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,2).getFill());

    //assume code steps foward
    javafxRun(() -> test1Percolution.step());

    assertEquals(full, thisGridView.getRectangle(0,0).getFill());
    assertEquals(full, thisGridView.getRectangle(0,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(0,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(1,0).getFill());
    assertEquals(full, thisGridView.getRectangle(1,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(1,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,0).getFill());
    assertEquals(empty, thisGridView.getRectangle(2,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,2).getFill());
    ...
    assertEquals(blocked, thisGridView.getRectangle(1,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,0).getFill());
    assertEquals(full, thisGridView.getRectangle(2,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,2).getFill());
  }
}
