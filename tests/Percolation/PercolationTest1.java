...
public class PercolationTest1 extends DukeTest {
  ...
  public void start(Stage stage) throws IOException {
    thisFileReader = new ReadCSVFile("Examples/Percolation/Test1.csv");
  ...
  public void testFile() throws IOException {
    ...
    int[][] testArray = {{0,1,0},{0,2,1},{0,0,0}};
  ...
  public void testGridData() throws IOException {
    ...
    List<Integer> knownNeighbors = new ArrayList<Integer>(Arrays.asList(0,1,0,0,1,0,0,0));
    ...
    for(CellModel neighbor:testNeighbors2 ){
  ...
  public void testGridModel() throws IOException {
    ...
    javafxRun(() -> {
      try {
        test1Percolution.restartSimulationWithNewProp("Percolation.properties");
      } catch (IOException e) {
        e.printStackTrace();
      }
    });
    ...
    assertEquals(0, thisGridModel.getCell(1,0).getState());
    assertEquals(2, thisGridModel.getCell(1,1).getState());
    assertEquals(2, thisGridModel.getCell(1,2).getState());
    assertEquals(0, thisGridModel.getCell(2,0).getState());
    assertEquals(0, thisGridModel.getCell(2,1).getState());
    assertEquals(0, thisGridModel.getCell(2,2).getState());
  ...
  public void testGridView() throws IOException {
    ...
    assertEquals(empty, thisGridView.getRectangle(1,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,0).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,1).getFill());
    ...
    assertEquals(blocked, thisGridView.getRectangle(1,0).getFill());
    assertEquals(full, thisGridView.getRectangle(1,1).getFill());
    assertEquals(full, thisGridView.getRectangle(1,2).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,0).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,1).getFill());
    assertEquals(blocked, thisGridView.getRectangle(2,2).getFill());
